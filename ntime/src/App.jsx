...
export default function App() {
  ...
  const saveList = () => {
    const errors = {};
    if (!listForm.name.trim()) errors.name = "Nome da lista é obrigatório.";
    if (Object.keys(errors).length) { setListErrors(errors); return; }
  ...
  const saveTask = () => {
    const errors = {};
    if (!taskForm.title.trim()) errors.title = "Título é obrigatório.";
    if (!taskForm.startDate)    errors.startDate = "Data de início é obrigatória.";
    if (!taskForm.deadline)     errors.deadline = "Prazo é obrigatório.";
    if (Object.keys(errors).length) { setTaskErrors(errors); return; }

    setLists(prev => {
      const next = prev.map((l, i) => {
        if (i !== currentList) return l;
